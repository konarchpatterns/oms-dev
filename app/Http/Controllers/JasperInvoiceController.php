...
class JasperInvoiceController extends Controller
{
  ...
  public function accountadmin()
  {
    ...
    $prv_mnth_vector = Order::where('order_us_date', '>=', $prv_startdt)
      ->where('order_us_date', '<=', $prv_enddt)
      ->where('status', '=', 'Completed')
      ->where('file_type', '=', 'Vector')
      ->where('allocation', 'like',  $allocname)
      ->sum('file_count', 'as', 'totvect');

    $prv_mnth_vector = collect($prv_mnth_vector);

    $prv_mnth_digit = Order::where('order_us_date', '>=', $prv_startdt)
      ->where('order_us_date', '<=', $prv_enddt)
      ->where('status', '=', 'Completed')
      ->where('file_type', '=', 'Digitizing')
      ->where('allocation', 'like',  $allocname)
      ->sum('file_count', 'as', 'totdigit');

    $curr_mnth_vector = Order::where('order_us_date', '>=', $curr_mnthdt)
    ...
    $curr_mnth_digit = Order::where('order_us_date', '>=', $curr_mnthdt)
      ->where('order_us_date', '<=', $curr_dt)
      ->where('status', '=', 'Completed')
      ->where('file_type', '=', 'Digitizing')
      ->where('allocation', 'like',  $allocname)
    ...
    // $query = Order::select('client_company', 'AS', 'f1')->where('allocation', '<>', "'$qblank'")
    //   ->where('allocation', '<>', "'$qzero'")
    //   ->where('allocation', '<>', "'$qadmin'")
    //   ->whereIn('status', ['Completed', 'Rev-Completed', 'Ch-Completed'])
    //   ->where("bill_dt", ">=", "'$prv_startdt'")
    //   ->where("bill_dt", "<=", "'$prv_enddt'")
    //   ->where('file_type', '=', "Vector")
    //   ->sum('file_price', 'as', "f2")->tosql();
  ...
  public function orderreport(Request $request)
  {
    ...
    if ($request->dformat == 'Download') {
      ...
      $orderlist = DB::table('orders')->select(
        'bill_dt',
        'client_company',
        'client_name',
        ...
        'file_price',
        'status'
      )->where('bill_dt', '>=', $pfr_dt)->whereIn('orders.status', ['Completed', 'Rev-Completed', 'Ch-Completed'])->orderBy('id', 'desc')->get();
    ...
    if ($ext == "csv") {
      ...
      if ($bulk == 'yes') {
        $entitiesArray = DB::table('client_dtls')->select(
          'client_dtls.client_id',
          'client_dtls.client_creation_id',
          'client_dtls.client_name',
          'client_dtls.client_email_primary',
          'client_dtls.client_company',
          'client_dtls.client_contact_1',
          'client_dtls.client_creation_date_time',
          'client_dtls.client_note',
          'orders.bill_dt',
          'orders.order_id',
          'orders.file_count'
        )->join('orders', 'orders.client_creation_id', 'client_dtls.client_creation_id')->where('orders.bill_dt', '>=', $pfr_dt)->where('orders.bill_dt', '<=', $pto_dt)->where('orders.file_count', '>', 1)->distinct()->get();
      } else {
  ...
  public function search(Request $request)
  {
    ...
    if ($request->has('pcomp')) {
      ...
      if (count($comp_names) == 0) {
        ...
        foreach ($comp_names as $comp) {
          //dd($comp->company_id);
          $comp_found = DB::table('invoice_summary')->select('company_id')
            ->where('yr_month', '=', $pyear . $mon)
            ->where('company_id', $comp->company_id)->count();
  ...
  public function  CompanyNotOrder(Request $request)
  {
    ...
    $companies = DB::table('clients')->select('client_company', 'company_id')->where('delete_flag', '<>', 'Y')->where('red_list', '<>', 'Y')->whereNotIn('company_id', function ($query) use ($pfr_dt, $pto_dt) {
      $query->select('company_id')->from('orders')->whereBetween('order_us_date', [$pfr_dt, $pto_dt]);
    })->get();
    ...
    if (count($companies) > 0) {
      ...
      $CsvData = array('Company Name,Last Order Id, Last Order Us Date,File Type, File Name, Last Status, Last Client Name, Email, Contact');
      foreach ($companies as $value) {
        // $CsvData[]=$value->company_id.','.$value->client_company;
  ...
  public function  CompanyNotOrder_future(Request $request)
  {
    //dd($request->all());
    $pfr_dt =  str_replace('/', '-', $request->pfr_dt);
    $pfr_dt = Carbon::parse($pfr_dt)->format('Y-m-d H:i');
    ...
    if (count($companies) > 0) {
      ...
      foreach ($companies as $value) {
        ...
        foreach ($lastorder as $lastdtl) {
          ...
          $contacts =  DB::table('client_dtls')->select(
            'client_contact_1',
            'black_list',
            'black_list_reason',
            'cunitno',
          ...
          )->where('client_creation_id', '=',  $last_creation_id)->get();
    ...
    if ($pinvdtoption == '0') {
      ...
      foreach ($comp_names as $cl) {
        ...
        $jasper->process(
          ...
          array(
            'driver' => 'mysql', 'host' => '192.168.0.10', 'port' => '3306',
