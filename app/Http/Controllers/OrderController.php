...
class OrderController extends Controller
{
  ...
  public function  index(Request $request)
  {
    ...
    $users = DB::table('users')->join('role_user', 'role_user.user_id', '=', 'users.id')->join(
      'sessions',
      'sessions.user_id',
      '=',
      'users.id'
    )->whereIn('role_user.role_id', array(6, 7))->pluck('users.name', 'users.id')->toArray();
      ...
      if (Auth::user()->hasRole('Designer') && Auth::user()->level() <= 11) {
        ...
        return Datatables::of($data)
          ...
          ->addColumn('order_id', function ($user) {
            ...
            return  view('partials.datatableseditorder', [
              'id' => $user->id, 'orderid' => $user->order_id, 'status' => $user->status, 'fileprice' => $user->file_price,
              'file_count' => $user->file_count,
              'tot_revision' => $tot_revision,   'vendors' => $vendors
            ])->render();
          })
        ...
        return Datatables::of($data)
          ...
          ->filterColumn('alloc_to_person', function ($query, $keyword) {
            $query->whereRaw("allocation like ?", ["%{$keyword}%"]);
          })
          ->addColumn('alloc_to_person', function ($user) {
            $files = explode(',', $user->allocation);
            $files = DB::table('order_dtls')->where('master_id', '=', $user->id)
    ...
    $noresponse =  DB::table('orders')->where('status', 'Quote')->where('created_at', '<=', $new_date)->select('id')->get()->toArray();
    ...
    if ($quotecount > 0) {
      DB::table('orders')->where('status', 'Quote')->whereIn('id', $quote)->update(['status' => 'No Response']);
    }
  ...
  <div class="modal fade dddd" id="alloccontmodal' . $value->id . '" style="overflow-y:hidden;">
    <div class="modal-dialog  modal-md" >
      ...
      if (
        $actstatus == 'Quote' || $actstatus == 'Alloted' || $actstatus == 'Ch-Alloted' ||
  ...
  public function store(Request $request)
  {
    ...
    $result = DB::transaction(function () use ($request) {
      try {
        ...
        $company =  DB::table('client_dtls')->select(
          'company_id',
          'client_note',
          'client_contact_1'
        )->where('client_creation_id', '=', $clid)->first();
        ...
        $parent_company =  DB::table('clients')->select(
          'client_address1',
          'client_state',
          'client_country',
          'digit_rate'
        ...
        Order::where('id', $id)->update([
          'file_name' => $all_filenames, 'note' => $all_fnotes, 'document_type' => $doc_type, 'target_completion_time' => $target_date,
      ...
      try {
        ...
        for ($i = 0; $i < $j; $i++) {
          ...
          if (isset($input_dtls['allocation'][$i])) {
            ...
            if (
              in_array('not allocated', explode(",", $input_dtls['allocation'][$i])) &&
            ...
            //// second condition
            if (
              in_array('0', explode(",", $input_dtls['allocation'][$i])) &&
              ($input_dtls['status'][$i] == 'QC Pending' || $input_dtls['status'][$i] == 'QC OK'
                || $input_dtls['status'][$i] == 'Alloted' || $input_dtls['status'][$i] == 'Completed')
            ) {
          ...
          if (isset($input_dtls['odtlid'][$i])) {
            ...
            $order_oldchild_record = DB::table('order_dtls')->where(
              'id',
              $child_id
            )->get();
        ...
        $child =  DB::table('order_dtls')->where(
          'master_id',
          '=',
          $child_id1
        )->where('deleted_flag', '<>', 'Y')->pluck('status', 'status')->toArray();

        $last_date =  DB::table('order_dtls')->where(
          'master_id',
          '=',
          $child_id1
        )->where('deleted_flag', '<>', 'Y')->max('target_completion_time');
        ...
        if ($finalstatus) {
          ...
          $Order->update([
            'file_name' => $all_filenames, 'note' => $all_fnotes, 'document_type' => $doc_type, 'file_count' => $j, 'allocation' => $all_alloc, 'alloc_id' => $all_allocid, 'child_status' => $child_status,
            'target_completion_time' => $last_date,
            'status' => $f_status,  'old_status' => $Order->status,
            'order_completion_time' => $compl_time
          ]);
          ...
          DB::table('user_logs')->where('id', $logid)->update([
            'file_name' => $all_filenames, 'note' => $all_fnotes, 'document_type' => $doc_type, 'target_completion_time' => $target_date, 'file_count' => $j,
          ...
          $Order->update([
            'file_name' => $all_filenames, 'note' => $all_fnotes, 'document_type' => $doc_type,   'target_completion_time'
  ...
  public function updatedtl(Request $request)
  {
    ...
    if (Auth::user()->level() < 20) {
      if (isset($request->statusshow)) {
        ...
        if (
          $new_status2  == 'Completed' || $new_status2  == 'Rev-Completed' ||
    ...
    if (
      isset($request->orddtls["chkid"][0]) &&  $request->allocshow <> 'yes' &&
      ...
      for ($i = 0; $i < $j; $i++) {
        ...
        if (in_array($input_dtls['childid'][$i], $input_dtls['chkid'])) {   // dd(($input_dtls['childid'][$i]));
          // dd($dtl);
      ...
      if (in_array('Changes', $child)) {
        ...
        $Order->update([
          'status' => $f_status,   'child_status' => $childstatus, 'allocation' => $allocation, 'alloc_id' => $allocationid,
          'old_status' => $Order->status
        ]);
          ...
          for ($i = 0; $i < $j; $i++) {
            ...
            if ($valueid == $input_dtls['childid'][$i]) {
              ...
              if (($input_dtls['status'][$i] == 'Approved') && ($alloc <> 'not allocated')) {
                ...
                $status = 'Alloted';
                $dtlid->update([
                  'allocation' =>  $allocf, 'alloc_id' => $alloc_id,
                  'status' => $status
                ]);
              } else {
          ...
          if ($valueid == $input_dtls['childid'][$i]) {
            ...
            switch ($new_status2) {
              ...
              case "Alloted":
                ...
                $chngedeallotedstatusobj->update([
                  'status' => $new_status2,
                  'old_status' => $oldstatus
                ]);
  ...
  public function updateorderstatus(Request $request)
  {
    ...
    if (Auth::user()->level() < 20) {
      ...
      if (
        $new_status2  == 'Approved' || $new_status2  == 'Completed' || $new_status2  == 'Revision' ||   $new_status2  == 'Duplicate Entry' ||
    ...
    if ($new_status2 == 'Approved') {
      ...
      if ($new_status2 == 'Completed') {
        ...
        OrderDtl::where('master_id', $id)
          ->update([
            'old_status' => $oldstatus, 'status' => $new_status2, 'updated_by' => $userid,
            'ip_address' => $ip_address,
            'status_change_date' => $cdt,
            'order_completion_time' => $cdt
          ]);
      ...
      if ($new_status2 == 'Rev-QC OK') {
        ...
        $note =  DB::table('user_allocation_log')->select('username', 'alloc_person', 'status')->where(
          'order_master_id',
          '=',
          $orderid->id
        )->where('status', '=', 'Rev-QC Pending')->orderby('id', 'asc')->get()->toArray();
  ...
  public function  updatereason(Request $request)
  {
    ...
    if ($orddtlcount == $orddtlcount_st) {
      ...
      if ($new_status2 == 'Approved'  || $f_status == 'Approved') {
        ...
        Order::where('id', $id)->update([
          'status' => $f_status,   'child_status' => $childstatus,
          'old_status' => $oldstatus
        ]);
      }
    }
  ...
  public function orderdtlstatus(Request $request)
  {
    ...
    if ($orddtlcount == $orddtlcount_st) {
      ...
      // dd($f_status);
      Order::where('id', $orderid->master_id)->update([
        'status' => $f_status,   'child_status' => $childstatus,
        'old_status' => $oldstatus
      ]);
