        ...
        if ($request->ajax()) {    
           ...
           ->addColumn('action', function ($user) {
              ...
              ->addColumn('file1', function ($user) {
                   $url =  "files/1/" . $user->yr_month ;
                   $url1 = "/" .$user->file_url ;
                   $url2 = $url.$url1;

                  // new changes below made to avoid unnecessary open of file on 02-feb-22'
                if (file_exists($url2)) {
                    // echo "The file $filename exists";
                      return  '<a href="'. $url2 .'"> '. $user->file_url .' </a>'; 
                } else {
                      //echo "The file $filename does not exist";
                      return "File not found" ."<br>". " or under preparation";
                    }
                 //return  '<a href="'. $url2 .'"> '. $user->file_url .' </a>';
...
public function UpdateDtls(Request $request, $id) 
{
 ...
 // dd($remain_amt);
  ...
  // $updated = array('pay_channel'=> $request->pay_channel,'amt_received_inr'=> $request->amt_received_inr,
