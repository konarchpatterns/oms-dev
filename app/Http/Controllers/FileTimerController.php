...
class FileTimerController extends Controller
{
   ...
   public function  index(Request $request) {
      ...
      $roleid = [] ;
      //if (Auth::user()->hasRole('Designer') ) {  commented as role id required for all users on 30/03/18
            //dd('ok');die;
                //$role = Role::where('name','=', 'Designer')->get();  removedo n 30/03/18
                //$role = Role::where('name','=', 'Designer')->get();  removed on 03-04-18  as will not work for other roles
      ...
      // dd($rights);
        //rights
         ...
         return view('timers.index', compact('users','user_names', 'status', 'rights', 'perms'));
   }  
     ...
     //  dd($diffcol1);
	  //Updated by logic added on 12/10/2017
       ...
       //  $childid =  [$childid1];
        ...
        return view('orders.edit', compact( 'Order', 'OrderDtl', 'users',  'status','perms','rights','vendors' , 'us_time', 'cdt' , 'target_date', 'created_byname' ,  'cols',        'updated_byname'));
...
public  function editdtl($id)
{
       ...
       // dd($orddtlcount);
        ...
        if (Auth::user()->hasRole('Designer') && Auth::user()->level()< 12) {
            ...
            $OrderDtl =  DB::table('order_dtls')->where('master_id' ,'=', $Order->id)->where('deleted_flag', '<>','Y')->whereRaw("FIND_IN_SET(?, alloc_id) > 0", [$id1])->get();
             //dd($OrderDtl);
        }
        else {  
               ...
               // dd($request->search);
                ...
                foreach ($clients as $client) {
                   ...
                   // dd($parent_company->digit_rate);
                    $digit_rate = 'null' ;
                    if($client->black_list == 'Y'){
                      $block='N';
                    }
                    elseif($client->black_list == 'N'){
                      $block='Y';
                    }
                    else{
                      $block='N';
