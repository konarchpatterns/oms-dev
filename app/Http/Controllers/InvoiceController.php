...
class InvoiceController extends Controller
{
  ...
  public  function  GenInvoice_konetk(Request $request)
  {
    ...
    $orderlist = DB::table('orders')->select(
      ...
      'file_price',
      'status'
    )->where('bill_dt', '>=', '2021-01-01')->whereIn('orders.status', ['Completed', 'Rev-Completed', 'Ch-Completed'])->where('company_id', '=', 'CO2017070013')->orderBy('id', 'desc')->get()->toArray();
    ...
    if ($pbutton == '1' ||  $pbutton == '2') { //  for  new records
      ...
      $invoice_copy = DB::table('invoice')
        ->select(
          'company_id',
          'client_company',
          DB::raw('SUM(file_price) as inv_amount'),
          DB::raw('SUM(inv_paid_amt) as paid_amt'),
    ...
    ")
      ...
      foreach ($invoice_copy->chunk(40) as $chunk) {
        foreach ($chunk as $key => $value) {
          ...
          $clientinfo = DB::table('table_client_b')->select(
            'client_company',
            'phone',
            'client_address1',
            'client_state',
          ...
          $invoiceInfo = DB::table('invoice_summary')->select(
            'invoice_no',
            'invoice_dt',
            'discount',
            'discount_reason'
          )->where('company_id', '=', $company_id)->where('yr_month', $yearmonth)->get()->toArray();
        ...
        foreach ($invoice_copy->chunk(2) as $chunk) {
          foreach ($chunk as $key => $value) {
            ...
            if ($count < 200 ||  $count > 400) {
              ...
              CreateInvoiceJobSecond::dispatch(
                $dcount,
                $orderlist,
                $invoiceInfo,
                $clientinfo,
  ...
  public function  InvoiceVectorPrint(Request $request)
  {
    ...
    $todt =  substr($request->yr_month, 0, 4) . '-' . substr($request->yr_month, 4, 2) . '-' . '30';
    ...
    $due_dt =  $new_dt1->subDays(-10)->format('d-M-Y');
    ...
    $invoiceInfo = DB::table('invoice_summary')->select(
      ...
      'discount',
      'discount_reason'
    )->where('company_id', '=', $company_id)->where('yr_month', $yrmonth)->get()->toArray();

    $ordersum = DB::table('orders')->select(DB::raw('DISTINCT file_type, sum(file_price) as file_price'))->where('bill_dt', '>=', $frdt)->where('bill_dt', '<=', $todt)->where(
      'company_id',
      '=',
      $company_id
    )->whereIn('orders.status', ['Completed', 'Rev-Completed', 'Ch-Completed'])->GroupBy('file_type')->get();
    ...
    $orderlist = DB::table('orders')->select(
      ...
      'file_price',
      'status'
    )->where('bill_dt', '>=', $frdt)->where('bill_dt', '<=', $todt)->where('company_id', '=',  $company_id)->whereIn('orders.status', ['Completed', 'Rev-Completed', 'Ch-Completed'])->orderByRaw('file_type DESC, client_name ASC')->get()->toArray();
  ...
  public function InvoiceDigitPrint()
  {
    ...
    $invoiceInfo = DB::table('clients')->select(
      'client_company',
      'phone',
      'client_address1',
      'client_state',
      'website',
      'client_country'
    )->where('company_id', '=', 'CO2019021573')->get()->toArray();
    ...
    $orderlist = DB::table('orders')->select(
      ...
      'file_price',
      'status'
    )->where('bill_dt', '>=', '2021-01-01')->where('bill_dt', '<=', '2021-05-31')->where('company_id', '=', 'CO2019021573')->whereIn('orders.status', ['Completed', 'Rev-Completed', 'Ch-Completed'])->orderByRaw('file_type DESC, client_name ASC')->get()->toArray();
  ...
  public function StoreSendEmail(Request $request)
  {
    ...
    Mail::send('emails.sample', [
      'toemail' => $toemail, 'frusernm' => $frusernm, 'tousernm' => $tousernm, 'subject' => $subject, 'year' => $year,
      'month' => $month, 'fileurl' => $fileurl
    ], function ($m) use ($frusernm, $toemail, $tousernm, $subject, $year, $month, $fileurl) {
